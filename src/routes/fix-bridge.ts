...
export async function fixBridgeRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  // Hedge executions — order lifecycle from hedge_records (authoritative status,
  // includes FAILED/REJECTED orders TE never confirmed). Replaces correlated-fills
  // for the ER grid load.
  fastify.get(
    '/fix/lp/:lp_id/hedge-executions',
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { lp_id } = lpIdParams.parse(request.params);
      const q = request.query as Record<string, string | undefined>;
      const qs = new URLSearchParams();
      if (q.limit) qs.set('limit', q.limit);
      const qstr = qs.toString() ? `?${qs.toString()}` : '';
      const response = await nexriskApi.get(`/api/v1/fix/lp/${lp_id}/hedge-executions${qstr}`);
      return emptyOn404(response, lp_id, [], reply);
    }
  );
  // ════════════════════════════════════════════════════════════
